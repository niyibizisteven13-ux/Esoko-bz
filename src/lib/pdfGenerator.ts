...
export const generateReceipt = (data: ReceiptData, options: { save?: boolean } = {}): Blob => {
  ...
  doc.text('ESOKO WALLET', pageWidth / 2, 20, { align: 'center' });
  ...
  doc.text('Thank you for using ESOKO Wallet.', pageWidth / 2, pageWidth > 250 ? 280 : 270, {
    align: 'center',
  });
  doc.text(
...
export const generateAccountingReport = (
  ...
  doc.text('This report is computer-generated by ESOKO Nexus.', pageWidth / 2, footerY, {
    align: 'center',
  });
  doc.text(`Report ID: ${reportId}`, pageWidth / 2, footerY + 5, { align: 'center' });

  doc.save(`ESOKO_${title.replace(/\s+/g, '_')}_${reportId}.pdf`);
};
